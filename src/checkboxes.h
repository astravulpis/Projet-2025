...
typedef struct checkbox {
    ...
    SDL_FRect *tickBox; // j'ai hésité a créer un FRect pour cela, mais c'est plus pratique, si c'est un symbole '✓' en image,
                        // comme ça il peut dépasser de la taille de la checkbox

    SDL_Texture *checkboxImg;
    SDL_Texture *tickImg;
    SDL_Texture *tickHoverImg;
    // image qui est une version 'fantôme' de tick_img, ou n'importe quoi d'autre, dans l'idée cela doit être transparent, cela
    // s'affiche quand on survole la checkbox et qu'elle n'est pas checked, partage le même box que tickImg
