...
void healthBarRender(sdl_ctx_t *sdl_ctx, health_bar *h, float hpValue, int s_intensity, int l_intensity, int ls_opacity)// s = shadow et l = ligth
{
    int healthTextWidth = 0;
    int healthTextHeight = 0;

    // Compare hpValue a la valeur maximum de la barre, copie la valeur das hp, la rabaisse si trop grande, la met a 0 si négative
    // cela ne se fait que sur la jauge, pas le pourcentage affiché (cela me parraît utile d'avoir la vrai valeur qui est mise en paramètre pour un potentiel debug)
    float hp = hpValue;
    if (hpValue > h->maxHp)
        hp = h->maxHp;
    else if (hpValue < 0)
    ...
    V2f barTextPosShadow = {(h->healthBarBox)->x + XCentering + 2 * sdl_ctx->screenRatio, (h->healthBarBox)->y + YCentering + 1 * sdl_ctx->screenRatio};

    // Calcul de fillBox via barPadding (normalement plus petite que h->healthBarBox, sinon il y'a un problème XD), largeur calculées en fonction de hp, représente la jauge de vie
    ...
    SDL_FRect emptyFillBox = {fillBox.x + fillBox.w, fillBox.y, ((h->healthBarBox)->w - (h->barPadding * sdl_ctx->screenRatio)) / h->maxHp * (h->maxHp - hp), fillBox.h};
    SDL_Color emptyBgColor = {(h->bgColor.r + 20) < 255 ? (h->bgColor.r + 20) : 255,
                              (h->bgColor.g + 20) < 255 ? (h->bgColor.g + 20) : 255,
                              (h->bgColor.b + 20) < 255 ? (h->bgColor.b + 20) : 255,
                               200};
    
    // rectangles et couleur de rendu pour effets d'ombres sur la barre de vie
    SDL_FRect horizontal_shadowFillBox = {fillBox.x, fillBox.y + fillBox.h - (h->barPadding * sdl_ctx->screenRatio), fillBox.w, h->barPadding * sdl_ctx->screenRatio};
    SDL_FRect vertical_shadowFillBox = {fillBox.x, fillBox.y, h->barPadding * sdl_ctx->screenRatio, fillBox.h};
    SDL_Color shadowColor = {((h->fillColor.r - s_intensity) >= 0) && ((h->fillColor.r - s_intensity) <= 255) ? (h->fillColor.r - s_intensity) : 0,
                             ...
                             ((h->fillColor.b - s_intensity) >= 0) && ((h->fillColor.b - s_intensity) <= 255) ? (h->fillColor.b - s_intensity) : 0,
                              (ls_opacity >=0 && ls_opacity <=255) ? ls_opacity : 255};// si l'opacité n'est pas bonne, alors l'ombre n'aura aucune transparence
    ...
    SDL_FRect vertical_lightFillBox = {fillBox.x + fillBox.w - (h->barPadding * sdl_ctx->screenRatio), fillBox.y, h->barPadding * sdl_ctx->screenRatio, fillBox.h};
    SDL_Color lightColor = {((h->fillColor.r + l_intensity) >= 0) && ((h->fillColor.r + l_intensity) <= 255) ? (h->fillColor.r + l_intensity) : 255,
                            ...
                            ((h->fillColor.b + l_intensity) >= 0) && ((h->fillColor.b + l_intensity) <= 255) ? (h->fillColor.b + l_intensity) : 255,
                             (ls_opacity >=0 && ls_opacity <=255) ? (ls_opacity * (hp / h->maxHp)): 255};// si l'opacité n'est pas bonne, alors l'ombre n'aura aucune transparence
    
    // initialisations des boxs et couleurs pour le curseur et ses ombres/ lumières
    SDL_FRect cursorBox = {fillBox.x + fillBox.w - (h->barPadding * sdl_ctx->screenRatio / 2), h->healthBarBox->y + (6 * sdl_ctx->screenRatio) / 2, h->barPadding * sdl_ctx->screenRatio / 2, h->healthBarBox->h - (6 * sdl_ctx->screenRatio)};
    
    SDL_FRect cursorShadowBox = {cursorBox.x, cursorBox.y, cursorBox.w / 3, cursorBox.h};
    SDL_Color cursorShadowColor = {50, 50, 50, 100};// valeurs en dur, je vais peut être revenir dessus, mais il me semble que les barres de vies sont déja assez personnalisables

    SDL_FRect cursorLightBox = {cursorBox.x + cursorBox.w - (cursorBox.w / 3), cursorBox.y, cursorBox.w / 3, cursorBox.h};
    SDL_Color cursorLightColor = {205, 205, 205, 100};// valeurs en dur, je vais peut être revenir dessus, mais il me semble que les barres de vies sont déja assez personnalisables
